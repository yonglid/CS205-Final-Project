...
int main(int argc, char **argv) {
	...
	while (m < T+1) {
		...
		for (int i = 1; i < N; i++) {
			...
			float val = H_old[i]*V_old[i]*V_old[i]*(1-V_old[i])/t_in - V_old[i]/t_out;
			V_new[i] = (((V_old[i+1] - 2*V_old[i] + V_old[i-1])*(K/(spacestep*spacestep))) + val)*timestep + V_old[i];
		}
